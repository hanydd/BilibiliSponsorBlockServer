...
export async function postWarning(req: Request, res: Response): Promise<Response> {
    ...
    try {
        if (enabled) {
            const previousWarning = (await db.prepare(
                "get",
                'SELECT * FROM "warnings" WHERE "userID" = ? AND "issuerUserID" = ? AND "type" = ?',
                [userID, issuerUserID, type]
            )) as warningEntry;
            ...
            if (!previousWarning) {
                ...
                await db.prepare("run", 'UPDATE "warnings" SET "enabled" = 1, "reason" = ? WHERE "userID" = ? AND "issueTime" = ?', [
                    reason,
                    userID,
                    previousWarning.issueTime,
                ]);
